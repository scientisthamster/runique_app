...
class SyncRunWorkerScheduler(
    ...
    override suspend fun scheduleSync(type: SyncRunScheduler.SyncType) {
        when (type) {
            ...
            is SyncRunScheduler.SyncType.DeleteRun -> scheduleDeleteRunWorker(type.id)
            is SyncRunScheduler.SyncType.CreateRun -> scheduleCreateRunWorker(
                run = type.run,
                mapPictureByteArray = type.mapPictureByteArray
            )
        }
    }

    private suspend fun scheduleDeleteRunWorker(id: String) {
        ...
        val workRequest = OneTimeWorkRequestBuilder<DeleteRunWorker>()
            ...
            .setInputData(
                Data.Builder()
                    .putString(CreateRunWorker.RUN_ID, id)
                    .build()
            )
            .build()
    ...
    private suspend fun scheduleCreateRunWorker(run: Run, mapPictureByteArray: ByteArray) {
        ...
        pendingSyncDao.upsertPendingSyncEntity(pendingRun)
